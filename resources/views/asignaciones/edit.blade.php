    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bi bi-pencil"></i> Editar Asignación de Vivienda</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('asignaciones.update', $asignacion) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('inspector_id') is-invalid @enderror" 
                                ...
                                <option value="{{ $inspector->id }}" 
                                        {{ old('inspector_id', $asignacion->inspector_id) == $inspector->id ? 'selected' : '' }}>
                                    {{ $inspector->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('vivienda_id') is-invalid @enderror" 
                                ...
                                <option value="{{ $vivienda->id }}" 
                                        {{ old('vivienda_id', $asignacion->vivienda_id) == $vivienda->id ? 'selected' : '' }}>
                        ...
                        @else
                        <!-- Inspectores solo ven la información, no pueden editarla -->
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Inspector</label>
                            <input type="text" class="form-control" value="{{ $asignacion->inspector->name }}" readonly>
                        </div>
                        
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Vivienda</label>
                            <input type="text" class="form-control" value="{{ $asignacion->vivienda->codigo }} - {{ $asignacion->vivienda->direccion }}" readonly>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" 
                                   ...
                                   value="{{ old('fecha_asignacion', $asignacion->fecha_asignacion->format('Y-m-d')) }}" 
                                   {{ auth()->user()->role !== 'administrador' ? 'readonly' : 'required' }}>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" 
                                   ...
                                   id="fecha_limite" 
                                   name="fecha_limite" 
                                   value="{{ old('fecha_limite', $asignacion->fecha_limite?->format('Y-m-d')) }}"
                                   {{ auth()->user()->role !== 'administrador' ? 'readonly' : '' }}>
                            @error('fecha_limite')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            @if(auth()->user()->role === 'administrador')
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select @error('estado') is-invalid @enderror" 
                                ...
                                <option value="en_progreso" {{ old('estado', $asignacion->estado) == 'en_progreso' ? 'selected' : '' }}>En Progreso</option>
                                <option value="completada" {{ old('estado', $asignacion->estado) == 'completada' ? 'selected' : '' }}>Completada</option>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <select class="form-select @error('prioridad') is-invalid @enderror" 
                                    ...
                                    name="prioridad" 
                                    required>
                                <option value="baja" {{ old('prioridad', $asignacion->prioridad) == 'baja' ? 'selected' : '' }}>Baja</option>
                                <option value="media" {{ old('prioridad', $asignacion->prioridad) == 'media' ? 'selected' : '' }}>Media</option>
                                <option value="alta" {{ old('prioridad', $asignacion->prioridad) == 'alta' ? 'selected' : '' }}>Alta</option>
                                <option value="urgente" {{ old('prioridad', $asignacion->prioridad) == 'urgente' ? 'selected' : '' }}>Urgente</option>
                            </select>
                            @error('prioridad')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('notas') is-invalid @enderror" 
                                  ...
                                  name="notas" 
                                  rows="4"
                                  placeholder="Agregue notas o instrucciones especiales para el inspector...">{{ old('notas', $asignacion->notas) }}</textarea>
                        @error('notas')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
